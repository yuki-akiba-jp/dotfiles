class SegmentTree:
	...
	def update(self, sell_index, value):
		...
		while sell_index >= 2:
			sell_index //= 2
			self.sells[sell_index] =self.sells[sell_index * 2]+ self.sells[sell_index * 2 + 1]
	...
	def query(self, left, right, sell_left, sell_right, sell_index):
		...
		answer_l = self.query(left, right, sell_left, m, sell_index * 2)
		answer_r = self.query(left, right, m, sell_right, sell_index * 2 + 1)
